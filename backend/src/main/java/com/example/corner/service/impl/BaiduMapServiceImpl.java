...
public class BaiduMapServiceImpl implements BaiduMapService {
    ...
    public Map<String, Object> reverseGeocoding(BigDecimal latitude, BigDecimal longitude) {
        ...
        try {
            // 构建请求URL
            String url = String.format("%s?ak=%s&output=json&coordtype=wgs84ll&location=%s,%s",
                    geocodingUrl, apiKey, latitude, longitude);
    ...
    public Map<String, Object> getLocationByIp(String ip) {
        ...
        try {
            // 构建请求URL
            String url = String.format("%s?ak=%s&ip=%s&coor=bd09ll",
                    ipLocationUrl, apiKey, ip != null ? ip : "");
